...
use common\models\questTask\WeightTask;
use yii\base\Action;
use Yii;
use yii\web\HttpException;

class WeightAction extends Action
{
    public function run($id)
    {
        ...
        if ($r1 && $r2 && $item->validate()) {
            ...
            try {
                ...
                $QuestEvent = QuestEvent::findOne($item->event_id);
                if(!$QuestEvent) {
                    throw new HttpException('Предмет не найден');
                }

                $item->setEventName($QuestEvent->name);
        ...
        return $this->controller->render('weight/create', [
            'model' => $model,
            'item'  => $item,
            'pocket' => $Pocket,
        ]);
