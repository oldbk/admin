...
/* @var $item \common\models\questTask\FightTask */
...
<div class="quest-part-form">
    ...
    <?= $form->field($item, 'fight_type')->textInput(['class' => 'form-control']) ?>

    <?= $form->field($item, 'fight_comment')->textInput(['class' => 'form-control']) ?>

    <?= $form->field($item, 'enemies')->textInput(['placeholder' => 'Опоненты через запятую. ЛОГИНЫ']) ?>
    <?= $form->field($item, 'not_enemies')->textInput(['placeholder' => 'Не должны быть в опонентах через запятую. ЛОГИНЫ']) ?>
