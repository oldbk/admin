...
/* @var $item \common\models\questTask\GiftTask */
...
<div class="quest-part-form">
    ...
    <?= $form->field($item, 'user_to')->textInput() ?>

    <?= $form->field($item, 'diff_persons')->checkbox(['class' => 'js-switch_2']) ?>
    <?= $form->field($item, 'only_male')->checkbox(['class' => 'js-switch_2']) ?>
    <?= $form->field($item, 'only_female')->checkbox(['class' => 'js-switch_2']) ?>
    <?= $form->field($item, 'only_level')->checkbox(['class' => 'js-switch_2']) ?>
    <div style="display: none;" id="only-level-option">
        <?= $form->field($item, 'min_level')->textInput(['class' => 'form-control touchspin1']) ?>
        <?= $form->field($item, 'max_level')->textInput(['class' => 'form-control touchspin1']) ?>
    </div>
    <?= $form->field($item, 'align')->textInput() ?>
    <?= $form->field($item, 'is_fshop')->checkbox(['class' => 'js-switch_2']) ?>
    <?= $form->field($item, 'is_give')->checkbox(['class' => 'js-switch_2']) ?>
...
<script>
    $(function() {
        $('#gifttask-only_level').on('change', function(){
            if($(this).prop('checked')) {
                $('#only-level-option').show();
            } else {
                $('#gifttask-min_level').val('');
                $('#gifttask-max_level').val('');
                $('#only-level-option').hide();
            }
        });
